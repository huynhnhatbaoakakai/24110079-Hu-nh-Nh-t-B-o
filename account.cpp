...
class Account {
    ...
    Account(string n , double b){
        accountNumber = n ; 
        balance = b ; 
    }// check thử coi tài khoản có đủ 10 chữ số không nếu không đủ thì có thể là tài khoản không thấy hoặc không hợp lệ 
       bool coPhaiACkh() {
    ...
    }// check coi tài khoản có đúng tài khoản ngân hàng không có thì in ra acnumber và số dư
      void displayBalance() {
         if (!coPhaiACkh()) {
